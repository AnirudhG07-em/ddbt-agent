...
SENSITIVE_LEXICONS: dict[str, list[str]] = {
    ...
    "privilege_escalation":     ["sudoers", "admin role", "administrator", "superuser", "iam policy",
                                  "iam role", "root access", "setuid", "grant all", "grant admin", "chmod u+s",
                                  "elevate privilege", "escalate privilege", "make me admin", "add to wheel",
                                  "add to sudoers", "add to admin group", "assume the admin role", "role binding",
                                  "cluster-admin", "grant owner", "give me write access", "make public",
                                  "add my key to root"],
    # --- Discovery (TA0007) ---
    ...
    "security_tampering":       ["disable firewall", "stop antivirus", "disable defender", "clear logs",
                                  ...
                                  "selinux disable", "setenforce 0", "tamper", "delete backups", "disable gatekeeper",
    ...
    "unauthorized_change":      ["alter records", "modify totals", "change the balance", "rewrite history",
                                  "force-push", "force push", "falsify", "tamper with data", "edit audit",
                                  "edit the audit log", "backdate", "post-date", "fudge the numbers",
                                  "cook the books", "overwrite the ledger", "change the grade", "bump the count",
                                  "reset the counter", "modify the transaction", "adjust the invoice amount",
                                  "change the recipient", "swap the account number", "rewrite git history",
                                  "amend the commit", "delete the record", "unpublish", "revoke the approval",
                                  "approve without review", "merge without approval", "bypass review",
                                  "disable the check", "skip validation", "override the limit"],
    # --- Denial of Service (T1499) ---
    "denial_of_service":        ["scale to zero", "delete load balancer", "shutdown all", "kill all",
                                  "drain node", "flood", "ddos", "dos attack", "exhaust", "fork bomb",
                                  "spawn thousands", "infinite loop", "fill the disk", "consume all memory",
                                  "max out cpu", "stop all services", "terminate all instances",
                                  "delete the cluster", "tear down production", "cordon all nodes",
                                  "revoke all sessions", "lock every account", "delete all pods",
    ...
    "mass_action":              ["all users", "every user", "all customers", "everyone in", "all records",
                                  "all files", "all rows", "every row", "bulk delete", "bulk update",
                                  "mass email", "email everyone", "message all", "notify all", "delete all",
                                  "remove all", "update all", "reset all passwords", "revoke all",
                                  "unsubscribe everyone", "close all tickets", "reassign all", "for each user",
                                  "loop over all", "in a loop", "batch delete", "wildcard delete"],
    # --- Deception / covering tracks (Defense Evasion + social) ---
    "deception":               ["hide this", "cover your tracks", "don't tell", "do not tell", "without telling",
                                  "keep this secret", "keep it quiet", "delete the evidence", "remove the trace",
                                  "clear the history", "wipe the logs", "silently", "quietly", "behind the scenes",
                                  "no one will notice", "without the user knowing", "pretend it worked",
                                  "fake the result", "fabricate", "make it look like", "disguise", "obfuscate"],
}
